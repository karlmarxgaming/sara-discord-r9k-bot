module.exports = {
    ...
    run: async function(handler, msg, args, output) {
        ...
        if(!content.length) return output.send({embeds: [client.embed.invalid('Please include a phrase to check!', 'Missing arguments!', this, msg.channel.guild.id)]});
        ...
        client.sql.selectWhere('messageData', 'content', compressed, null, 1).then(([rows]) => {
            ...
            output.send({embeds: [client.embed.info(`Your phrase **has${!saidHere ? "n't" : ''}** been used in ${msg.guild.name} before!\n**${saidIn}** other Guild${saidIn == 1 ? ' has' : 's have'} seen this phrase!`, 'Availability')]});
        });
    }
}