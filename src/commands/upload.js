...
module.exports = {
    ...
    run: function(handler, msg, args, output) {
        ...
        if(!attachment || !attachment.name || !attachment.name.endsWith('.r9k')) return output.send({embeds: [client.embed.invalid('You must attach a valid .r9k file to your message!', 'Invalid arguments', this, msg.channel.guild.id)]});

        const request = https.get(attachment.url, (res) => {
            const len = Number(res.headers['content-length']);
            if(!len || len < 40) return output.send({embeds: [client.embed.invalid('The provided .r9k file is corrupted or outdated.', 'Invalid .r9k file')]});
                ...
                if(!valid && bufIndex >= 8) {
                    ...
                    while(i-->0) {
                        if(buf[i] != MAGIC[i]) {
                            ...
                            request.abort();
                            return output.send({embeds: [client.embed.invalid('The provided .r9k file is corrupted or outdated.', 'Invalid .r9k file')]});
                        }
                    }
                    valid = true;
                }
            ...
            async function handleBuffer() {
                try {
                    ...
                    if(await client.misc.confirm(output, msg.member.id, `This Dataset was provided by **${user.tag}** and is **${client.time(Date.now() - json.timestamp)}** old!\n\nUploading this dataset will completely overwrite the current one. Do you wish to proceed? \n(This action can **not** be undone)`, 'Overwrite dataset?')) {
                        ...
                        client.sql(query)
                        .then(() => {
                            output.send({embeds: [client.embed.success('Successfully updated the dataset!', 'Success')]});
                        }).catch(handler);
                    };
                } catch(err) {
                    handler.output = null;
                    handler(err);
                    output.send({embeds: [client.embed.invalid('The provided .r9k file is corrupted or outdated.', 'Invalid r9k file')]});
                }
            }
