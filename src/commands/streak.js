module.exports = {
    ...
    run: async function(handler, msg, args, output) {
        ...
        if(args.length) {
            ...
            if(!member) return output.send({embeds: [client.embed.invalid('Could not locate the specified user, make sure they are in this guild!', 'Unknown User')]});
        }
        ...
        output.send({embeds: [client.embed.info(str, 'Mute Streak')]});
    }
}