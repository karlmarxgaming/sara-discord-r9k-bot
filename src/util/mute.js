module.exports = {
    ...
    unmuteUpdateChannelPerms: function(userId, guildId) {
        return new Promise(async (resolve, reject) => {
            ...
            for(let i = 0; i < channels.length; ++i) {
                ...
                promises.push(channel.permissionOverwrites.edit(member, this.unmuteObj));
            }
    ...
    muteUpdateChannelPerms: function(userId, guildId) {
        return new Promise(async (resolve, reject) => {
            ...
            for(let i = 0; i < channels.length; ++i) {
                const channel = guild.channels.cache.get(channels[i]);
                promises.push(channel.permissionOverwrites.edit(member, this.muteObj));
            }
            return Promise.all(promises)
            .then(resolve)
            .catch(reject);
    ...
    unmuteNotification: function(userId, guildId, applyId) {
        return new Promise(async (resolve, reject) => {
            ...
            member.send({embeds: [client.embed.info(`You have been unmuted from **${guild.name}**${applying ? ` by ${applying}` : ''} and can now use the ${channelStr} channel${channels.length == 1 ? '' : 's'} again!`, (applying ? 'Manually' : 'Automatically') + ' unmuted')]})
            .then(resolve)
    ...
    muteNotification: function(userId, guildId, len, applyId) {
        return new Promise(async (resolve, reject) => {
            ...
            member.send({embeds: [client.embed.info(`You have been muted${applying ? ` by ${applying}` : ''} in **${guild.name}** from the ${channelStr} channel${channels.length == 1 ? '' : 's'} for repeating a phrase!\n\nYour mute will expire in **${client.time(len)}**!`, (applying ? 'Manually' : 'Automatically') +  ' muted')]})
            .then(resolve)
            .catch(() => {}); // User has DMs disabled
